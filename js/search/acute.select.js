...
angular.module("acute.select", ['acute.select'])
.directive("acSelect", function($parse, acuteSelectService) {
    ...
    return {
        ...
        // **************************************************************
        controller('Trans2Ctrl', function ($scope, $http) {
...
$scope.TransportList2 =data;
 ...
 $scope.getAllTrans = function (callback) {
    callback($scope.TransportList2);
};

$scope.TransSelected = function (Trans) {
    $scope.TransInfo = Trans.Make + " (" + TransportList2.ID + ")";
};
